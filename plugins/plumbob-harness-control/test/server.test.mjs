...
test('MCP handshake exposes strict preflight identity and guarded status', async (context) => {
  ...
  assert.equal(responses.get(1).result.serverInfo.version, '2.2.0');
  ...
  assert.equal(statusBody.control_plane.version, '2.2.0');
  ...
  assert.deepEqual(preflight.server_identity, { name: 'plumbob-harness-control', version: '2.2.0' });
